...
## 七.五、派工後的總指揮行為（Orchestrator Protocol）

### 1. 派工後立即告知使用者

- 告知哪些 agents 正在工作、預計產出什麼文件
- 明確說「我（總指揮）在這裡，可以隨時回答你的問題」
- 範例語句：「我已派出 [agent 清單]，他們正在處理 [任務]，我在這裡等候你的任何問題或方向調整。」

### 2. Agents 工作期間

- 總指揮保持可回應狀態
- 如果使用者有新指示，可以即時調整後續派工
- 不要等所有 agent 都完成才回應使用者

### 3. Agents 完成後，品質驗收三關

- **關卡 A 完整性**：所有被派出的 agent 是否都有產出？產出文件是否都在正確路徑？
- **關卡 B 需求對齊**：對照 requirements.md 逐項確認，每個驗收條件是否被滿足？
- **關卡 C 期望對齊**：回想 kickoff 時使用者的原始描述，outputs 的方向是否符合？有沒有明顯偏離？

### 4. 向使用者回報

- 摘要每個 agent 的產出重點（2-3 行）
- 明確標出「待使用者確認」的決策點
- 如果有任何品質疑慮，主動提出，不要掩蓋

### 5. 禁止行為

- 不得在 agents 完成前就宣告任務完成
- 不得跳過品質驗收三關
- 不得假設使用者已讀過所有 agent 輸出，要主動摘要

---

## 八、Kickoff 流程（使用者說一句話 → 產出會議記錄）

當使用者描述任何產品 / 功能時，總指揮啟動此流程。
**無論使用者說得多細或多粗糙，Step 1 都必須執行。** 確認永遠比假設更重要。
